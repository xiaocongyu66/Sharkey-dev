...
const FB: Record<string, LangPack> = {
	...
	info: {
		en: 'Links local accounts by real IP and browser fingerprint, scores behavior (login bursts), optional OpenAI-compatible AI judgment, and can auto-suspend all linked accounts. Uses a separate API key from note moderation.',
		zh: '通过真实 IP 与浏览器指纹关联本站账号，结合登录频率等行为，可选调用独立 AI（OpenAI 兼容）判定是否刷号，并可将关联账号一并封禁。API Key 与帖子审核独立。',
		'zh-TW': '以真實 IP 與瀏覽器指紋關聯本站帳號，結合登入頻率等行為，可選獨立 AI 判定並批量封禁。',
		ja: '実IPと指紋でローカル垢を紐づけ、ログイン頻度等とAI判定で一括凍結可能。ノート審査とは別APIキー。',
	},
	...
	autoSuspend: { en: 'Auto-suspend linked accounts', zh: '自动封禁关联账号', 'zh-TW': '自動封禁關聯帳號', ja: '関連垢を自動凍結' },
	autoSuspendCaption: {
		en: 'When AI (or hard heuristic) flags abuse, suspend all linked non-moderator local accounts. Moderators/root are never auto-suspended.',
		zh: 'AI 或强启发式判定为滥用时，封禁所有关联的非版主本地账号。版主/root 不会被自动封。',
		'zh-TW': '判定濫用時封禁關聯的非版主本機帳號。',
		ja: '濫用判定時、関連する非モデレーターのローカル垢を凍結。',
	},
	...
	minLinkedCaption: {
		en: 'Need at least this many local accounts sharing IP/fingerprint before evaluating.',
		zh: '同一 IP/指纹下至少关联到这么多本站账号才开始评估。',
		'zh-TW': '同一 IP/指紋至少關聯這麼多本站帳號才評估。',
		ja: '同一IP/指紋でこの数以上のローカル垢がある時のみ評価。',
	},
