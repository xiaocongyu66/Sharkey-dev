				...
				<div :key="user.id" class="main _panel">
					<div class="banner-container" :class="{ [$style.bannerContainerTall]: useTallBanner }">
						...
						<div :class="$style.actions" class="actions">
							...
							<!-- Mutual follow: quick chat (local users) -->
							<button
								v-if="canQuickChat"
								v-tooltip="i18n.ts.chat"
								:class="$style.actionsChat"
								class="_button"
								@click="openChat"
							>
...
/** Mutual follow + local user + chat available → show quick chat */
const canQuickChat = computed(() => {
	if ($i == null) return false;
	if ($i.id === props.user.id) return false;
	if (props.user.host != null) return false; // local only
	if ($i.policies?.chatAvailability === 'unavailable') return false;
	// mutual follow required
	return props.user.isFollowing === true && props.user.isFollowed === true;
});

function openChat() {
	if (!canQuickChat.value) return;
	router.push(`/chat/user/${props.user.id}`);
}
