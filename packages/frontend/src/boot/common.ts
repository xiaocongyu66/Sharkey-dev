...
export async function common(createVue: () => Promise<App<Element>>) {
	...
	// Ensure chat appears in sidebar (desktop) / drawer for existing accounts
	try {
		const menu = prefer.s.menu ?? [];
		if (!menu.includes('chat')) {
			const idx = menu.indexOf('notifications');
			const next = [...menu];
			next.splice(idx >= 0 ? idx + 1 : 0, 0, 'chat');
