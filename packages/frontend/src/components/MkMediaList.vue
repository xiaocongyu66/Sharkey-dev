...
async function calcAspectRatio() {
	...
	// Single video without stored dimensions: force 16:9 so player is not a collapsed empty strip
	if (props.mediaList.length === 1 && img?.type?.startsWith('video') && !(img.properties?.width && img.properties?.height)) {
		gallery.value.style.aspectRatio = '16 / 9';
		return;
	}
	...
	switch (prefer.s.mediaListWithOneImageAppearance) {
		...
		case '2_3':
			...
			break;
		default:
			gallery.value.style.aspectRatio = img.type?.startsWith('video')
