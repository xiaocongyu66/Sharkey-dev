	...
	const spec = {
		...
		info: {
			...
			description: [
				...
				'## Interactive docs',
				'',
				`Human-readable explorer (login required): ${config.url}/api-doc`,
				'',
				'Machine-readable OpenAPI 3.1: `/api.json` (**requires authentication** — same Bearer token as other API calls).',
				'',
				'## Notes',
				'',
				...
				'- API catalog (`/api.json`, `/api/endpoints`, `/api/endpoint`, `/api-doc`) requires a valid access token (not public recon).',
			].join('\n'),
