...
export default class extends Endpoint<typeof meta, typeof paramDef> { // eslint-disable-line import/no-default-export
	constructor(
		...
		super(meta, paramDef, async (ps, me) => {
			...
			if (!meIsAdmin && await this.roleService.isAdministrator(user)) {
				throw new Error('cannot show info of admin');
			}

			// Login IP / fingerprint only for administrators (not chat roles / normal users / non-admin moderators)
			const signins = meIsAdmin
				? await promiseMap(
					await this.signinsRepository.find({
