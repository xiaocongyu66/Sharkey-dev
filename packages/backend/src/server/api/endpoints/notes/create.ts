...
import type { UsersRepository, NotesRepository, BlockingsRepository, DriveFilesRepository, ChannelsRepository, MiMeta } from '@/models/_.js';
...
export const meta = {
	...
	errors: {
		...
		localNoteCreationDisabled: {
			message: 'Note creation is temporarily disabled on this server.',
			code: 'LOCAL_NOTE_CREATION_DISABLED',
			id: 'c8a2e4f1-6b3d-4f9a-9e12-0a1b2c3d4e5f',
		},
	},
} as const;
...
export default class extends Endpoint<typeof meta, typeof paramDef> { // eslint-disable-line import/no-default-export
	constructor(
		...
		super(meta, paramDef, async (ps, me, _token, _file, _cleanup, ip, headers) => {
			// Site-wide local note creation ban (admins/moderators can still post)
			if (this.serverSettings.disableLocalNoteCreation) {
				const isMod = await this.roleService.isModerator(me);
				if (!isMod) {
					throw new ApiError(meta.errors.localNoteCreationDisabled);
				}
			}
			...
			try {
				const note = await this.noteCreateService.create(me, {
					...
					clientIp: ip ?? null,
					clientFingerprint: extractClientFingerprint(headers as any),
				});
