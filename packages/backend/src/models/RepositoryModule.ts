...
const $chatStickerPacksRepository: Provider = {
	provide: DI.chatStickerPacksRepository,
	useFactory: (db: DataSource) => db.getRepository(MiChatStickerPack).extend(miRepository as MiRepository<MiChatStickerPack>),
	inject: [DI.db],
};

const $chatStickersRepository: Provider = {
	provide: DI.chatStickersRepository,
	useFactory: (db: DataSource) => db.getRepository(MiChatSticker).extend(miRepository as MiRepository<MiChatSticker>),
	inject: [DI.db],
};

const $chatE2eeKeysRepository: Provider = {
	provide: DI.chatE2eeKeysRepository,
	useFactory: (db: DataSource) => db.getRepository(MiChatE2eeKey).extend(miRepository as MiRepository<MiChatE2eeKey>),
	inject: [DI.db],
};
