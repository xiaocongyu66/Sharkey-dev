...
export class MiNote {
	...
	/** Client IP at creation time (local notes only; admin moderation). */
	@Column('varchar', {
		length: 128,
		nullable: true,
	})
	...
	/** Browser fingerprint at creation time (local notes only; admin moderation). */
	@Column('varchar', {
		length: 256,
		nullable: true,
	})
	...
	/**
	 * Soft-hidden by staff. Invisible to everyone except instance administrators/moderators.
	 * Also set when an account is suspended (posts remain hidden).
	 */
	@Column('boolean', {
		default: false,
	})
