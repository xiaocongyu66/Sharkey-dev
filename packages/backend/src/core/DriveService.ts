...
export class DriveService {
	...
	private async save(file: MiDriveFile, path: string, name: string, info: FileInfo): Promise<MiDriveFile> {
		...
		//#region ObjectStorage params
			...
			// Thumbnail / webpublic use their own media type extensions (e.g. webp), not the original's
			const thumbnailAccessKey = makeFileKey(alts.thumbnail?.ext ?? ext, 'thumbnail');
			const webpublicAccessKey = makeFileKey(alts.webpublic?.ext ?? ext, 'webpublic');
