...
export class UserSuspendService {
	...
	@bindThis
	public async suspend(user: MiUser, moderator: MiUser, opts?: { hideNotes?: boolean; hideChat?: boolean }): Promise<void> {
		...
		// Hide all posts by this user (staff-only visibility). Default on.
		if (opts?.hideNotes !== false) {
			await this.db.query(
				`UPDATE "note" SET "isHidden" = true WHERE "userId" = $1 AND "isHidden" = false`,
				[user.id],
			);
		}

		// Optionally hide chat messages they sent
		if (opts?.hideChat === true) {
			await this.db.query(
				`UPDATE "chat_message" SET "isHidden" = true WHERE "fromUserId" = $1 AND "isHidden" = false`,
				[user.id],
			);
		}
